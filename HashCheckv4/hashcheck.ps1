...
#            try {
                ...
                Write-Output ""  >> "C:\HashCheck\logs\hashcheck_log_$date.txt"
                    ...
                    if ($hashstuff1 -eq $hashstuff2) {
                        ...
                        Write-Output ""  >> "C:\HashCheck\logs\hashcheck_log_$date.txt"
                        #Send-MailMessage -SmtpServer "ML370" -From "ConsistencyCheck@fcclabs.local" -to npasqualetti@fcclabs.local -subject "Consistency Check SUCCESS for $($sourceHashLocation)" -Body "Successful consitency check from $($sourceHashLocation) to $($destinationHashLocation). This email is automated, please direct questions to help@fcclabs.local"
                    }else{
                        Write-Output "Sad.. Consistency check failed and I'm going to send an email!" >> "C:\HashCheck\logs\hashcheck_log_$date.txt"
                        Write-Output "" >> "C:\HashCheck\logs\hashcheck_log_$date.txt"
                        Send-MailMessage -SmtpServer "ML370" -From "ConsistencyCheck@fcclabs.local" -to npasqualetti@fcclabs.local -subject "Consistency Check FAILED for $($sourceHashLocation)" -Body "Failed consitency check from $($sourceHashLocation) to $($destinationHashLocation). If there was an execption previous to receiving this email, you can ignore. If not, ensure that the computer is powered on, locked-up, needs a restart, etc. This email is automated from FCC-Utility-1 C:\HashCheck, please direct questions to help@fcclabs.local"
                    }

            }#catch{

                #Write-Host "BOOO! Catch this error"  
                #Send-MailMessage -SmtpServer "ML370" -From "ConsistencyCheck@fcclabs.local" -to help@fcclabs.local -subject "Consistency Check EXCEPTION for $($sourceHashLocation)" -Body "Failed consitency check from $($sourceHashLocation) to $($destinationHashLocation). Ensure that files are not in use. This email is automated from FCC-Utility-1 C:\HashCheck, please direct questions to help@fcclabs.local"
        #}
    #}
}

Send-MailMessage -SmtpServer "ML370" -From "ConsistencyCheck@fcclabs.local" -to npasqualetti@fcclabs.local -subject "Consistency Check Log $date" -Body "This email is automated from FCC-Utility-1 C:\HashCheck, please direct questions to help@fcclabs.local" -Attachments "C:\HashCheck\logs\hashcheck_log_$date.txt"
